...
form1 <- function(dataf1) {
    ...
    # ===
    crop.est.method <- ifelse(!is.na(dataf1[25, 7]), "Transplanted", 
                              ifelse(!is.na(dataf1[28, 7]), "Direct_seeded", NA
                                                                            ))
    ...
    # ===
    type.var <- ifelse(!is.na(dataf1[32, 13]), "traditional",
                       ifelse(!is.na(dataf1[32, 20]), "modern", 
                              ifelse(!is.na(dataf1[32, 26]), "hybrid", NA
                                     )))
    ...
    CropStatus <- ifelse(!is.na(dataf1[88, 8]), "very good", 
                         ifelse(!is.na(dataf1[88, 16]), "good",
                                ifelse(!is.na(dataf1[88, 26]), "average", 
                                       ifelse(!is.na(dataf1[90, 8]), "poor", 
                                              ifelse(!is.na(dataf1[90, 16]), "very poor", NA
                                                     )))))
    ...
    form1data <- cbind(field.no, 
                       ...
                       latitude,
                       longitude,
                       field.area, farmer.name, land.form, prev.crop, fallow.prd, 
                       soil.salt, soil.zinc, soil.alum, 
                       crop.est.method, seed.age,
                       crop.est.date,
                       crop.harv.date,
                       rice.var,
                       type.var,
                       N1application, P1application, K1application, N2application, P2application, K2application, N3application, P3application, K3application, N4application, P4application, K4application,
                       Ntotal, Ptotal, Ktotal,
                       N1Name, P1Name, K1Name, N2Name, P2Name,K2Name,  N3Name, P3Name, K3Name, N4Name,P4Name, K4Name,
                       Chem1Name, Chem2Name, Chem3Name, Chem4Name, Chem5Name, Chem1Dose, Chem2Dose, Chem3Dose,Chem4Dose, Chem5Dose, Chem1Date, Chem2Date, Chem3Date, Chem4Date, Chem5Date,
                       CropStatus,
                       yld.area1, yld.area2, yld.area3
        )
