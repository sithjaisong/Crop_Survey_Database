...
for (j in 2:length(dirlist)) {
    ...
    for (i in 1:length(list.file)) {
        ...
        if (is.na(check.df2.1) == TRUE | is.na(check.df2.2) == TRUE) {
            ...
            print(paste(list.file[i], "is not complete because either visit or visit2 has not data",
                sep = " "))
            next  # mean run
            # print('RUN')
        } else {
            ...
            # arrange the column by moving the filename to the first, and visit and
            # DVS inthe second and third, respectively.
            ...
            FORM2 <- merge_recurse(form2.list, by = c("filename", "visit",
                "DVS"))
        }
    }
}

filenames <- FORM2$filename
...
FORM2$index <- filenames

col_idx <- grep("index", names(FORM2))

FORM2 <- FORM2[, c(col_idx, (1:ncol(FORM2)[-col_idx]))]

save(FORM2, file = "~/Google Drive/surveySKEP1/database1.RData")

## == output is new.row.sum head(FORM2.new) # the final output

# save the output write.csv(FORM2, 'FORM2.csv') # in case if you want
# to export the csv file eos
